  ...
  describe("Partner fee", () => {
    ...
    describe("Sell order", () => {
      it("Partner fee should be substracted from buy amount after network costs", () => {
        ...
        const partnerFeeAmount = Math.floor(
          (buyAmountBeforeNetworkCosts * partnerFeeBps) / 100 / 100
        );
    ...
    describe("Buy order", () => {
      it("Partner fee should be added on top of sell amount after network costs", () => {
        ...
        const partnerFeeAmount = Math.floor(
          (+orderParams.sellAmount * partnerFeeBps) / 100 / 100
        );
  ...
  describe("Slippage", () => {
    ...
    describe("Sell order", () => {
      it("Slippage should be substracted from buy amount after partner fees", () => {
        ...
        const slippageAmount =
          (buyAmountAfterNetworkCosts * slippagePercentBps) / 100 / 100;

        expect(Number(result.afterSlippage.buyAmount)).toBe(
          Math.ceil(buyAmountAfterNetworkCosts - slippageAmount)
        );
      });
    });
    ...
    describe("Buy order", () => {
      it("Slippage should be added on top of sell amount after partner costs", () => {
        ...
        const sellAmountAfterNetworkCosts =
          +orderParams.sellAmount + +orderParams.feeAmount;
        const slippageAmount =
          (sellAmountAfterNetworkCosts * slippagePercentBps) / 100 / 100;

        // We are loosing precision here, because of using numbers and we have to use toBeCloseTo()
        expect(Number(result.afterSlippage.sellAmount)).toBeCloseTo(
          sellAmountAfterNetworkCosts + slippageAmount,
          -2
        );
      });
    });

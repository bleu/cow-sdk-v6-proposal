...
const FETCH_RESPONSE_PARAMETERS = expect.objectContaining(
...
describe("CoW Api", () => {
  ...
  test("Valid: Get version", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.cow.fi/xdai/api/v1/version",
  ...
  test("Valid: Get an order for custom chainId", async () => {
    ...
    fetchMock.mockResponseOnce(
      JSON.stringify({ ...ORDER_RESPONSE, class: "limit" }),
      {
        status: HTTP_STATUS_OK,
        headers: HEADERS,
      }
  ...
  test("Invalid: Get an order", async () => {
    ...
    // given
    fetchMock.mockResponse(JSON.stringify(errorBody), {
  ...
  test("Valid: Get last 5 orders for a given trader ", async () => {
    ...
    fetchMock.mockResponse(JSON.stringify(ORDERS_RESPONSE), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
  ...
  test("Valid: Get last 5 trades for a given trader ", async () => {
    ...
    fetchMock.mockResponse(JSON.stringify(TRADES_RESPONSE), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
  ...
  test("Valid: Send sign order cancellation", async () => {
    fetchMock.mockResponseOnce(JSON.stringify(SIGNED_ORDER_RESPONSE), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
    ...
    const body = {
      ...SIGNED_ORDER_RESPONSE,
      orderUids: [ORDER_CANCELLATION_UID],
    };
  ...
  test("Valid: Send an order ", async () => {
    fetchMock.mockResponseOnce(JSON.stringify("validOrderId"), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
  ...
  test("Valid: Get last 5 orders changing options parameters", async () => {
    const ORDERS_RESPONSE = Array(5).fill(ORDER_RESPONSE);
    fetchMock.mockResponseOnce(JSON.stringify(ORDERS_RESPONSE), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
    const orders = await orderBookApi.getOrders({
  ...
  test("Valid: Get last 5 trades changing options parameters", async () => {
    const TRADES_RESPONSE = Array(5).fill(TRADE_RESPONSE);
    fetchMock.mockResponseOnce(JSON.stringify(TRADES_RESPONSE), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
    const trades = await orderBookApi.getTrades({
  ...
  test("Valid: Get AppData", async () => {
    // given
    const appDataHash =
      "0x1fddf237451709522e5ac66887f979db70c3501efd4623ee86225ff914423fa1";
    const appDataBody = {
  ...
  test("Valid: Upload AppData", async () => {
    ...
    // when
    const appDataHashResult = await orderBookApi.uploadAppData(
      appDataHash,
      appDataBody.fullAppData
    );
  ...
  test("Valid: Get solver competition by tx hash", async () => {
    // given
    const txHash =
      "0xe395eac238e7c6b2f4c5dea57d4a3d9a2b42d9f4ae5574dd003f9e5dd76abeee";
    fetchMock.mockResponseOnce(JSON.stringify(AUCTION), {
      status: HTTP_STATUS_OK,
      headers: HEADERS,
    });
